...
def find_mean(total_weight, number):
    mean = total_weight / number
    print(f"Mean = {mean}")

def find_median(number, sorted_data):
    if number % 2 == 0:
        median1 = float(sorted_data[number//2])
        median2 = float(sorted_data[number//2 - 1])
        median = (median1 + median2) / 2
    else:
        median = float(sorted_data[number//2])
...
find_mean(total_weight, number)
find_median(number, sorted_data)
